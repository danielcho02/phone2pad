...
//
// Usage: in exactly ONE .cpp, define PHANTOMPAD_TEST_MAIN before including this
...
#define TEST_CASE(NAME)                                                            \
    ...
    static ::phantompad::test::Registrar PP_CONCAT(pp_test_reg_, __LINE__)(        \
        NAME, &PP_CONCAT(pp_test_fn_, __LINE__));                                  \
    static void PP_CONCAT(pp_test_fn_, __LINE__)()

#define CHECK(COND)                                                                \
    do {                                                                           \
        ++::phantompad::test::check_count();                                       \
        if (!(COND)) {                                                             \
            ::phantompad::test::report_failure(__FILE__, __LINE__,                 \
                                               "CHECK(" #COND ") failed");         \
        }                                                                          \
    } while (0)
...
#define CHECK_EQ(A, B)                                                             \
    do {                                                                           \
        ...
        if (!(_a == _b)) {                                                         \
            ...
            ::phantompad::test::report_failure(__FILE__, __LINE__, _oss.str());    \
        }                                                                          \
    } while (0)

#ifdef PHANTOMPAD_TEST_MAIN
int main() {
    using namespace phantompad::test;
